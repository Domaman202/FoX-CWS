...
public class ClasspathModCandidateFinder implements ModCandidateFinder {
	...
	public void findCandidates(ModCandidateConsumer out) {
		if (Fox.FABRIC_MODS_ENGINE.isDevelopmentEnvironment()) {
			...
			try {
				Enumeration<URL> mods = Fox.FOX_CLASS_LOADER.getResources("fabric.mod.json");
	...
	private static Map<Path, List<Path>> getPathGroups() {
		...
		Set<Path> cp = new HashSet<>(Arrays.stream(Fox.FOX_CLASS_LOADER.getTransformationPaths()).map(it -> {
            try {
                return Path.of(it.toURI());
