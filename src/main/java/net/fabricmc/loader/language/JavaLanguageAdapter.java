...
public class JavaLanguageAdapter implements LanguageAdapter {
	private static boolean canApplyInterface(String itfString) throws IOException {
		...
		InputStream stream = Fox.FOX_CLASS_LOADER.getResourceAsStream(LoaderUtil.getClassFileName(itfString));
	...
	public static Class<?> getClass(String className, Options options) throws ClassNotFoundException, IOException {
		InputStream stream = Fox.FOX_CLASS_LOADER.getResourceAsStream(LoaderUtil.getClassFileName(className));
		...
		stream.close();
		return Class.forName(className, true, Fox.FOX_CLASS_LOADER);
	}
